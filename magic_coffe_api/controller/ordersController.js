...
exports.createOrder = async (req, res) => {
  const { items, branch_id, card_id, total_price } = req.body;
  const userId = req.user.id; // authMiddleware’dan

  try {
    // Validation
    if (!items || !Array.isArray(items) || items.length === 0 || !branch_id || !total_price) {
    ...
    try {
      ...
      const [orderResult] = await connection.query(
        'INSERT INTO orders (user_id, branch_id, card_id, total_price, status) VALUES (?, ?, ?, ?, ?)',
        [userId, branch_id, card_id || null, total_price, 'pending']
      );
      ...
      for (const item of items) {
        await connection.query(
          'INSERT INTO branch_orders (order_id, branch_id, coffee_id, quantity, price, selected_volume_ml, delivery_type, intensity, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [
            orderId,
            branch_id,
            item.coffee_id,
      ...
      res.status(201).json({ message: 'Sipariş oluşturuldu', order_id: orderId });
    } catch (error) {
